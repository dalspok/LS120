...
class Human < Player
  ...
  def choose
    ...
    loop do
      ...
      player_choice = gets.chomp.downcase.chr
      break if Move::CHOICES.keys.include? player_choice
      puts "Wrong choice, pleas try again."
    end
    self.choice = Move.new(Move::CHOICES[player_choice])
    system "clear"
  end
end
